...
echo "${BCORE}"
if [[ "$YEAR" == "2017" ]]; then
    BCORE="${BCORE} --mcc susy-sos/mcc-METFixEE2017.txt "; 
fi;
...
case $lepton in
mu) 
   BCORE="${BCORE} -E ^${lepton} --xf 'SingleEl.*,DoubleEG.*,EGamma.*'  "; NUM="muon_tight"; QCD=QCDMu; 
   conept="LepGood_pt" ;; 
el)
    BCORE="${BCORE} -E ^${lepton} --xf 'DoubleMu.*,SingleMu.*' ";  NUM="ele_tight"; QCD=QCDEl; 
    conept="LepGood_pt" ;;
...
case $trigger in
Mu3_PFJet40)
    BCORE="${BCORE} -A 'entry point' trigger 'HLT_${trigger}' -A 'entry point' recoptfortrigger 'LepGood_pt>13.0 && LepGood_awayJet_pt>45'  "; 
    ...
    ;;
Mu8)
    BCORE="${BCORE} -A 'entry point' trigger 'HLT_${trigger}' -A 'entry point' recoptfortrigger 'LepGood_pt>13' "; 
    PUW=" -L susy-sos/lepton-fr/frPuReweight.cc -W 'puw${trigger}_${YEAR}(PV_npvsGood)' "
    ;;
Mu17)
...
MuX_OR)
    ...
    CONEPTVAR="ptfine_conePt"
    PUW="-L susy-sos/lepton-fr/frPuReweight.cc -W 'coneptw${trigger}_${YEAR}($conept,PV_npvsGood)' "
    ;;
Ele8|Ele8_CaloIdM_TrackIdM_PFJet30)
    BCORE="${BCORE} -A 'entry point' trigger 'HLT_Ele8_CaloIdM_TrackIdM_PFJet30' -A 'entry point' recoptfortrigger 'LepGood_pt>8 '  "; 
    PUW=" -L susy-sos/lepton-fr/frPuReweight.cc -W 'puwEle8_${YEAR}(PV_npvsGood)' "
    ;;
Ele17|Ele17_CaloIdM_TrackIdM_PFJet30)
    ...
    PUW=" -L susy-sos/lepton-fr/frPuReweight.cc -W 'puwEle17_${YEAR}(PV_npvsGood)' "
    ;;
Ele23|Ele23_CaloIdM_TrackIdM_PFJet30)
    ...
    PUW=" -L susy-sos/lepton-fr/frPuReweight.cc -W 'puwEle23_${YEAR}(PV_npvsGood)' "
    ;;
EleX_OR)
...
#more=$4;
QCD_what=$4
PBASE="~/www/FakeRateData/2018/${ANALYSIS}/qcd1l/$lepdir/HLT_$trigger/$QCD_what/$what/"
echo $QCD_what
           ...
           el) 
               ...
               case $trigger in
                   Ele8)
                       ...
                       RANGES="$RANGES --xcut 0 40 " ;;
                   Ele17)
                       XVAR="${XVAR}"
                       #/_coarselongbin/_coarseel17bin}"
                       RANGES="$RANGES --xcut 10 40 " ;;
                   Ele23)
                       XVAR="${XVAR}"
                       #/_coarselongbin/_coarseel23bin}"
                       RANGES="$RANGES --xcut 20 100 " ;;
                   EleX_Combined)
                       ISCOMB=true
                       ISWIDE=true
           ...
           mu)
               ...
               case $trigger in
                   ...
                   Mu8)
                       ...
                       #ISWIDE=true
                       RANGES="${RANGES} --xcut 13 45";;
                   Mu17)
                       XVAR="${XVAR}"
                        #/_coarselongbin/_coarsemu17bin}"
                       RANGES="${RANGES} --xcut 0 100 ";;
                   Mu20)
                       XVAR="${XVAR}"
                       #/_coarselongbin/_coarsemu20bin}"
                       RANGES="${RANGES} --xcut 0 100 ";;
                   Mu27)
                       XVAR="${XVAR}"
                       #/_coarselongbin/_coarsemu27bin}"
                       RANGES="${RANGES} --xcut 0 100 ";;
                   #Mu50)
