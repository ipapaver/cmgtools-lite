...
class bTagWeightAnalyzer():
    ...
    def __init__(self, csv, eff, algo='CSVv2', wp=1, branchbtag='btagCSV', branchflavor='hadronFlavour', recllabel='Recl', isFastSim=False):
        ...
        if eff:
            ...
            self.h_eff_b = f_eff.Get("h_effM_b"); self.h_eff_b.SetDirectory(0)
            self.h_eff_c = f_eff.Get("h_effM_c"); self.h_eff_c.SetDirectory(0)
            self.h_eff_l = f_eff.Get("h_effM_l"); self.h_eff_l.SetDirectory(0)
            f_eff.Close()


	## __call__
    ## _______________________________________________________________
    def __call__(self, event, pname =""):
        self.event = event
        self.resetMemory()
        self.collectObjects()
        self.computeWeights()
    ...
    def collectObjects(self):
        ...
        if hasattr(self.event, "iJSel"): 
            self.jets = []
            indices = list(getattr(self.event, "iJSel"))[0:int(getattr(self.event,"nJetSel"))]
            for idx in indices:
    ...
    def computeWeights(self):
        ...
        central = (dataNoTag * dataTag ) / ( mcNoTag * mcTag ) if mcNoTag*mcTag != 0 else 0
    ...
    def getEff(self, pt, eta, mcFlavor):
       ...
       return theHisto.GetBinContent(max(1, min(theHisto.GetNbinsX(), theHisto.GetXaxis().FindBin(pt ))), \
                                     max(1, min(theHisto.GetNbinsY(), theHisto.GetYaxis().FindBin(eta))))
